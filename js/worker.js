...
function genTextBody(textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    if (textBodyNode["a:p"].constructor === Array) {
        ...
        text += "</div>";
        //  console.log("single p text", text)
    }
...
function genTextBody(textBodyNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    if (textBodyNode["a:p"].constructor === Array) {
        ...
        for (var i = 0; i < textBodyNode["a:p"].length; i++) {
            var pNode = textBodyNode["a:p"][i];
            var rNode = pNode["a:r"];
            html += "<div class='" + getHorizontalAlign(pNode, slideLayoutSpNode, slideMasterSpNode, type, slideMasterTextStyles) + "'>";
            html += genBuChar(pNode);
            if (rNode === undefined) {
                // without r
                let content = genSpanElement(pNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj);
                html += content.html;
                text_content += content.text_content;
            } else if (rNode.constructor === Array) {
                // with multi r
                for (var j = 0; j < rNode.length; j++) {
                    let content = genSpanElement(rNode[j], slideLayoutSpNode, slideMasterSpNode, type, warpObj);
                    html += content.html;
                    text_content += content.text_content;
                }
            } else {
                // with one r
                let content = genSpanElement(rNode, slideLayoutSpNode, slideMasterSpNode, type, warpObj);
                html += content.html;
                text_content += content.text_content;
            }
            html += "</div>";
        }
        // console.log("multi p text", text_content)
    } else {
        // one p
        var pNode = textBodyNode["a:p"];
        var rNode = pNode["a:r"];
        ...
        html += "</div>";
        // console.log("single p text", text_content)
    }
    text_content = text_content.replaceAll("&nbsp;", "");
    //console.log("formatted p text", text_content)

    if (currentslide in SlideContent) {
        var tempContent = SlideContent[currentslide];
        SlideContent[currentslide] = tempContent + text_content;
    }
    else {
        SlideContent[currentslide] = text_content;
    }
    return html;
}
...
function genSpanElement(node, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    // else
    //     console.log("text_content", type, text);
...
function genSpanElement(node, slideLayoutSpNode, slideMasterSpNode, type, warpObj) {
    ...
    var text = node["a:t"], text_content = node["a:t"];
    if (typeof text !== 'string') {
        text = getTextByPathList(node, ["a:fld", "a:t"]);
        if (typeof text !== 'string') {
            text = "&nbsp;";
    ...
    var linkID = getTextByPathList(node, ["a:rPr", "a:hlinkClick", "attrs", "r:id"]), html = "";
    if (linkID !== undefined) {
        var linkURL = warpObj["slideResObj"][linkID]["target"];
        html = "<span class='text-block " + cssName + "'><a href='" + linkURL + "' target='_blank'>" + text.replace(/\s/i, "&nbsp;") + "</a></span>";
    } else {
        html = "<span class='text-block " + cssName + "'>" + text.replace(/\s/i, "&nbsp;") + "</span>";
    }

    return {
        html,
        text_content: text_content.replace(/\s/i, "&nbsp;")
    }
